...
//======================================Product Functionality================================================
    ...
    public function editProduct($prodId)
    {
        ...
        $data['categoryData'] = $this->Seller_model->getAllCategories();
    ...
    {
        if ($this->Seller_model->editProductImage()) {
            $this->session->set_flashdata('res_img_err', 'Image Updated Successfully.');
            if (is_file(base_url() . 'images/products/' . $this->input->post('db_img'))) {
                echo base_url() . 'images/products/' . $this->input->post('db_img');
                unlink(base_url() . 'images/products/' . $this->input->post('db_img'));
            }
            redirect('se_products');
        } else {
            $this->session->set_flashdata('res_img_err', 'Image Update Failed.');
        }
    }

    public function changeProductStatus($logStatus, $prodId)
    {
        if ($this->Seller_model->changeProductStatus($logStatus, $prodId)) {
            redirect('se_products');
        } else {
            //echo "error";
            redirect('se_products');
