...
class Super_admin extends CI_Controller
{
    ...
    {
        $data['deliveryBoysData'] = $this->Main_model->getAllDeliveryBoysData();
        $this->load->view('super_admin/delivery_boys_list', $data);
    }
    ...
    public function addDeliveryBoy()
    {
        ...
        $this->load->view('super_admin/add_delivery_boy', $data);
    }

    public function addNewDeliveryBoy()
    {
        $this->form_validation->set_rules('dboy_name', 'Delivery Boy Name', 'required');
        $this->form_validation->set_rules('dboy_mobile', 'Mobile Number', 'required|regex_match[/^[0-9]{10}$/]|is_unique[delivery_boys.dboy_mobile]');
        $this->form_validation->set_rules('dboy_address', 'Address', 'required');
        $this->form_validation->set_rules('dboy_email', 'Email', 'required|valid_email|is_unique[delivery_boys.dboy_email]');

        if (empty($_FILES['dboy_photo']['name'])) {
            $this->form_validation->set_rules('dboy_photo', 'Photo', 'required');
        }
        if (empty($_FILES['dboy_id_proof']['name'])) {
            $this->form_validation->set_rules('dboy_id_proof', 'ID Proof Image', 'required');
        }

        if ($this->form_validation->run() == FALSE) {
            $this->load->view('super_admin/add_delivery_boy');
        } else {
            $statusArray = $this->Main_model->addNewDeliveryBoy();

            if ($statusArray['status']) {
                $this->session->set_flashdata('res_reg_err', $statusArray['message']);
                $this->load->view('super_admin/delivery_boys_list');
            } else {
                $this->session->set_flashdata('res_reg_err', $statusArray['message']);
                $this->load->view('super_admin/delivery_boys_list');
            }
    ...
    public function editDeliveryBoy($dboyId)
    {
        $data['deliveryBoyData'] = $this->Main_model->getDeliveryBoyDataById($dboyId);
        $this->load->view('super_admin/edit_delivery_boy', $data);
    }

    public function updateDeliveryBoy($dboyId)
    {
        $this->form_validation->set_rules('dboy_name', 'Delivery Boy Name', 'required');
        $this->form_validation->set_rules('dboy_mobile', 'Mobile Number', 'required|regex_match[/^[0-9]{10}$/]');
        $this->form_validation->set_rules('dboy_address', 'Address', 'required');
        $this->form_validation->set_rules('dboy_email', 'Email', 'required|valid_email');

        if ($this->form_validation->run() == FALSE) {
            $this->load->view('super_admin/add_delivery_boy');
        } else {
            $statusArray = $this->Main_model->updateDeliveryBoy($dboyId);
    ...
    {
        $delStatus = $this->Main_model->deleteDeliveryBoy($dboyId);
        if ($delStatus == "success") {
            redirect('su_DeliveryBoys');
        } else {
            $this->session->set_flashdata('res_reg_err', "Operation Failed");
            redirect('su_DeliveryBoys');
        }
    }
